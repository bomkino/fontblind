...
def ensure_changelog() -> None:
    ...
    marker = "## Unreleased\n\n- Add one verified Linux browser-app runtime"
    if marker in text:
        return
    ...
    path.write_text(block + text[len("# Changelog\n"):].lstrip("\n"), encoding="utf-8")
...
def ensure_readme() -> None:
    ...
    text = text.replace(
        ...
        "FontBlind is one local-only font utility with three tools. The native wrapper targets macOS; Linux uses the same hardened runtime in a browser-hosted AppImage or portable bundle:",
        1,
    )
    if "## Linux\n" not in text:
        marker = "## Download\n"
...
def main() -> int:
    ...
    remove = (
        ...
        ".github/workflows/cleanup-linux-branch.yml",
        "tools/apply_linux_foundation.py",
        "tools/finalize_linux_foundation.py",
        "tools/cleanup_linux_branch.py",
        "docs/LINUX_SCOPE.md",
        ...
        "docs/LINUX_STATUS.md",
        "docs/LINUX_REVIEW.md",
        "docs/LINUX_FINAL_NOTE.md",
        "docs/LINUX_HANDOFF.md",
    )
